...
int main(int argc, char *argv[])
{
 ...
 md=atoi(argv[6]);
 if (cattitude<0) cattitude=0;
 if (cattitude>1) cattitude=1;
 ...
 for (int i=0; i<n_cats; i++)
 {
  ...
  while (!done)
  {
   ...
   if (done) {cats[i][0]=px;cats[i][1]=py;}
  }
 }
 for (int i=0; i<n_cheese; i++)
 {
  ...
  while (!done)
  {
   ...
   if (done) {cheese[i][0]=px;cheese[i][1]=py;}
  }
 }
 done=0;
 while (!done)
 {
  ...
  if (done) {mouse[0][0]=px;mouse[0][1]=py;}
 }
...
int checkForMouse(int x, int y)
{
 ...
 if (mouse[0][0]==x&&mouse[0][1]==y) return(1); else return(0);
}
...
int checkForCheese(int x, int y)
{
 ...
 for (int i=0; i<n_cheese; i++)
  if (cheese[i][0]==x&&cheese[i][1]==y) return(1);
 return(0);
}
...
int checkForCats(int x, int y)
{
 ...
 for (int i=0; i<n_cats; i++)
  if (cats[i][0]==x&&cats[i][1]==y) return(1);
 return(0);
}
   ...
   for (int j=0; j<size_Y; j++)
    if (MSTflg[i+(size_X*j)]==1)
     for (int k=0;k<4;k++)
      ...
      {
       if (k==0){idx=i;idy=j-1;}
       else if (k==1){idx=i+1;idy=j;}
       else if (k==2){idx=i;idy=j+1;}
       else {idx=i-1;idy=j;}
  ...
  for (int j=1;j<size_Y-1;j++)
   for (int k=0; k<4; k++)
    if (drand48()<loop_fraction)
    {
     ...
     if (k==0) MST[i+((j-1)*size_X)][2]=1.0;
     else if (k==1) MST[i+1+(j*size_X)][3]=1.0;
     else if (k==2) MST[i+((j+1)*size_X)][0]=1.0;
     else MST[i-1+(j*size_X)][1]=1.0;
    }
...
void updateFrame(int path[graph_size][2])
{
 ...
 if (s_mode<3)
 {
  ...
  for (int i=0; i<size_X; i++)
   for (int j=0; j<size_Y; j++)
    if (grid_value[i][j]>max_vis) max_vis=grid_value[i][j];
 ...
 else
 {
  ...
  for (int i=0; i<size_X; i++)
   for (int j=0; j<size_Y; j++)
   {
    if (grid_value[i][j]>max_util) max_util=grid_value[i][j];
    if (grid_value[i][j]<min_util) min_util=grid_value[i][j];
   }
 ...
 */
 ...
 if (xx1<0) xx1=0;
 if (xx1>=buf->sx) xx1=buf->sx;
 if (xx2<0) xx2=0;
 if (xx2>=buf->sx) xx2=buf->sx;
 if (yy1<0) yy1=0;
 if (yy1>=buf->sy) yy1=buf->sy;
 if (yy2<0) yy2=0;
 if (yy2>=buf->sy) yy2=buf->sy;
...
{
 if (key=='q') {free(catSprt.imgdata); free(mouseSprt.imgdata); free(cheeseSprt.imgdata); free(maze.imgdata); free(framebuffer.imgdata); exit(0);}
}
...
//  for (int i=0; i<ncheese; i++)
  for (int i=n_cheese-1; i>=0; i--)
  {
   ...
   {
    if (n_cheese>0) {cheese[i][0]=cheese[n_cheese-1][0];cheese[i][1]=cheese[n_cheese-1][1];}
    n_cheese--;
    if (n_cheese==0) endgame=1;
   }
  }
 ...
 else
 {
  ...
  {
   if (end_frames==0) fprintf(stderr,"Oh no! poor poor mouse!\n");
   if (frame%2) renderMaze(&maze,Graph,255,0,0);
   else renderMaze(&maze,Graph,0,0,255);
  }
  else 			           // Mouse won
  {
   if (end_frames==0) fprintf(stderr,"Happy, well-fed mouse!\n");
   if (frame%2) renderMaze(&maze,Graph,0,255,0);
   else renderMaze(&maze,Graph,0,0,255);   
  }
  end_frames++;
  if (end_frames>=10) {free(catSprt.imgdata); free(mouseSprt.imgdata); free(cheeseSprt.imgdata); free(maze.imgdata); free(framebuffer.imgdata); exit(0);}
 }
